#!/bin/bash
# MongoDB Monitoring Script using mongostat
# Usage: ./monitor-mongodb.sh <PASSWORD> [interval] [count]
# Example: ./monitor-mongodb.sh Svnglobal@2025 1 10
...
if [ -z "$PASSWORD" ]; then
  ...
  echo "Usage: ./monitor-mongodb.sh <PASSWORD> [interval] [count]"
  echo "Example: ./monitor-mongodb.sh Svnglobal@2025 1 10"
  echo ""
  echo "Arguments:"
...
URI="mongodb+srv://svnglobal:${ENCODED_PASSWORD}@svnglobal.5vlys7w.mongodb.net/?retryWrites=true&w=majority&appName=svnglobal"
...
CMD="mongostat --uri \"${URI}\" --humanReadable"
